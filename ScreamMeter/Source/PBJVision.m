    ...
    AVCaptureVideoDataOutputSampleBufferDelegate>
{
    ...
    CMFormatDescriptionRef _cmFormatDescriptionRef;//Amol added
...
- (id)init
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(changeFlashLevel:) name:@"ChangeFlashLevel" object:nil];
    }
    return self;
}

- (void) changeFlashLevel:(NSNotification *) notification{
    NSDictionary *userInfo = notification.userInfo;
    NSNumber *peakPowerNumber =[userInfo objectForKey:@"peakPowerForChannel"];
    float peakPowerForChannel = [peakPowerNumber floatValue];

    
    if([_captureDeviceBack isTorchAvailable] && [_captureDeviceBack isTorchModeSupported:AVCaptureTorchModeOn]){
        [_captureDeviceBack lockForConfiguration:nil];
        if (peakPowerForChannel==1.0f) {
            [_captureDeviceBack setTorchModeOnWithLevel:AVCaptureMaxAvailableTorchLevel error:NULL];
        }else if(peakPowerForChannel<0.3){
            [_captureDeviceBack setTorchMode:AVCaptureTorchModeOff];
            ...
            [_captureDeviceBack setTorchModeOnWithLevel:lightValue error:NULL];
            if (ScreamingGameplayState == ScreamingNotStartedState) {
                ScreamingGameplayState = ScreamingStartedState;
            }
        }
        [_captureDeviceBack unlockForConfiguration];
    }
}
    ...
    [self _enqueueBlockInCaptureVideoQueue:^{
        ...
        if (isVideo && isReadyToRecord && !_flags.interrupted) {
            ...
            if (desc->mChannelsPerFrame == 1 && desc->mBitsPerChannel == 16) {
                ...
                CMFormatDescriptionRef formatDescription = _cmFormatDescriptionRef;
                const AudioStreamBasicDescription* fmtDesc = CMAudioFormatDescriptionGetStreamBasicDescription (formatDescription);
                ...
                totalLeft=totalLeft/sampleCount;
    ...
    if (inputPort) {
        ...
        if (formatDescription) {
            _cleanAperture = CMVideoFormatDescriptionGetCleanAperture(formatDescription, YES);
              _cmFormatDescriptionRef = inputPort.formatDescription;
